...
for (int i = 0; i < 100; i++) {
	dub.push(rand.nextDouble()*100);
}
        // 3. Create a Queue of Doubles using the ArrayDeque class
        //    Note: you have to use the capitalized Double and not double
ArrayDeque<Double> qdub = new ArrayDeque<Double>();
...
String hRemove = "Removing " + h + " elements from Queue: ";
for (int i = 1; i <= h; i++) {
	hRemove+=qdub.remove() + " ";
}
System.out.println(hRemove);
...
while(qdub.size()<5) {
	...
	if(dub.isEmpty() == false) {
		qdub.add(dub.pop());
	}
} 
        // 7. Loop until there are no more elements in either the Stack or Queue
        //    and all the elements are printed
        for (int i = 0; i < dub.size()+qdub.size(); i++) {
			if(dub.isEmpty() == false && qdub.isEmpty() == false) {
        	System.out.println(dub.pop() + " " + qdub.remove());
